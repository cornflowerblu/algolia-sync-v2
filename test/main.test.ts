...
describe('Health check', () => {
  it('should return 200', async () => {
    ...
    expect(response.status).to.be.greaterThanOrEqual(200)
  })

  it('should return a timestamp that is not in the past', async () => {
    const response = await request.get('/health')
    expect(response.body.timestamp).to.greaterThan(Date.now() - 1000)
  })
})
...
describe('Sync', () => {
  ...
  it('should return 200 if a valid key is provided', async () => {
    ...
    expect(response.status).to.greaterThanOrEqual(200)
  })
})
